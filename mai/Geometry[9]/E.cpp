...
double distance_to_segment(vec3d p, vec3d q, vec3d a) {
    if (dot(q-p, a-p) < 0) return distance1(a, p);
    if (dot(p-q, a-q) < 0) return distance1(a, q);
    return distance_to_line(p, q, a);
}
double solve() {
    vec3d a, p, q;
    cin >> a.x >> a.y >> p.x >> p.y >> q.x >> q.y;
    return distance_to_segment(p, q, a);
}
int main() {
    int n;
    cin >> n;
